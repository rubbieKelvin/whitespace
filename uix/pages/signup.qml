...
MobilePage {
	...
	property SnackBar snack: snackbar
	...
	function do_signup() {
		...
		api.user.signup(password_input.text, function (xhr) {
			...
			snack.show("user created!")

			// now... login
			api.user.login(password_input.text, function (xhr) {
				//	snack.show("logged in as " + email_input.text.trim())
				app_stack.push("./home.qml")
			}, function () {
				snack.show("error logging in", "close")
			})
		}, function () {
			snack.show("error creating user!", "close")
	...
	Column {
		...
		Column {
			...
			FormComponents.TextInput {
				...
				background: Rectangle {
					color: "white"
					border.width: 1
					border.color: parent.focus ? ColorJS.color(ColorJS.primary) : "#CDCDCD"
					radius: 12
					anchors.fill: parent
				}
		...
		Column {
			...
			FormComponents.TextInput {
				...
				property bool password_hidden: true
				passwordCharacter: "*"
				echoMode: password_hidden ? TextInput.Password : TextInput.Normal
				rightButton.visible: true
				rightButton.flat: true
				rightButton.text: password_hidden ? "show" : "hide"

				Connections{
					target: password_input.rightButton
					function onClicked(){
						password_input.password_hidden = !password_input.password_hidden
					}
				}
		...
		Column {
			...
			CheckBox {
				id: checkBox
				height: 35
				text: qsTr("Accept terms & conditions")
			}
	...
	Item {
		...
		Image {
			...
			source: "../images/whitespace-signup-banner.svg"
...
/*##^##
Designer {
	D{i:0;autoSize:true;formeditorZoom:0.75;height:806;width:414}
}
##^##*/
